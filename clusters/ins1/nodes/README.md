...
Do not apply a worker manifest before its K3s agent has joined the cluster. A
Node manifest for an absent machine would create a phantom, permanently
`NotReady` Kubernetes Node. Host reconcile scripts join a worker first and
then apply its checked-in policy.
